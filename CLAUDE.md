...
# GitNexus — manual code-intelligence (human tool)

GitNexus is **not** an agent tool in this setup — it's a manual exploration + documentation
aid (web UI, wiki, architecture maps). Claude does not call it autonomously; it helps run it
on request via the `gnx` wrapper (on PATH):

- `gnx index <path>` — build/refresh the knowledge graph (+ embeddings) for any repo by path
- `gnx serve` — open the web UI at http://127.0.0.1:4747 (browses all indexed repos)
- `gnx wiki <path>` · `gnx status` · `gnx list` · `gnx doctor` · `gnx help`

For agentic *"what does this change touch?"* reasoning, use **`sem`** (always-fresh CLI, in
the pre-commit hook) — not GitNexus. Full reference: global `~/.claude/CLAUDE.md` → GitNexus.

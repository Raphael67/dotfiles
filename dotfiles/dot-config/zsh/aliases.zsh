...
alias gs='git status --short --branch && [[ -f .gitmodules ]] && git submodule foreach --quiet '\''printf "  %-25s %-15s %s\n" "$name" "$(git branch --show-current || echo "detached")" "$(git diff --quiet && git diff --cached --quiet && echo "clean" || echo "DIRTY")"'\'
